...
export function ProjectList() {
  ...
  const projects = useMemo(() => {
    ...
    const language = activeFilter as ProjectLanguage;
    return profile.projects.filter((project) =>
      (project.languages as readonly ProjectLanguage[]).includes(language)
    );
  }, [activeFilter]);

  return (
    <>
      <div className="mb-8 flex flex-wrap gap-2" aria-label="Project language filters">
        {filters.map((filter) => {
